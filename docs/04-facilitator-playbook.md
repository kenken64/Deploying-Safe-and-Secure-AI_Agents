...
```bash
git switch ollama-real-model-support     # the demos only land on the lab branch
...
**On the solution branch both of these stop**, which is the point of that branch and
useless as an opening demo. Check which branch you are on before the room fills.
