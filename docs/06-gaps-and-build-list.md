...
| 7 | **`SECURE_NO_CREDS_IN_STATE` is declared but not implemented** | It is one of the 18 controls in `config.py`, is listed in the Day 1 README and as control 11 in `03-security-reference.md` — and has no `vulnerable_*`/`secure_*` pair, no `settings.on()` call site, no attack naming it in `closed_by`, and no test. Toggling it in the control room changes nothing. Either build it (credentials fetched at the action, never placed in state) or drop it from the registry; a switch that does nothing teaches the wrong lesson. | half a day to build, 10 min to drop |
| 8 | **`fix it with:` can name the wrong controls** | `attacks/run.py` computes the advice line as `closed_by` minus the controls already on. But three controls are inert unless a prerequisite is on — `SECURE_AUTHZ` and `SECURE_TOOL_RESULTS` need `SECURE_EXECUTOR`, `SECURE_PRIV_SEP` needs `SECURE_QUARANTINE`. So `attack a2 --control SECURE_AUTHZ` lands and the advice **drops `SECURE_AUTHZ`**, and `attack a6 --control SECURE_TOOL_RESULTS` lands with an **empty** advice line. Subtract only controls that are on *and* effective, or name the missing prerequisite. Full evidence in `07-control-comparison.md`. | 2 hours |
