...
class MockLLM:
    ...
    def complete(self, context: list[Content], tools: list[dict]) -> Completion:
        ...
        if "do_lookup_other" in found:
            ...
            if m and not self._did(already, "lookup", m.group(1)):
        ...
        if ("do_send_external" in found or "do_exfiltrate" in found) and not self._did(already, "send_summary", ""):
    ...
    def _say(context: list[Content]) -> str:
        ...
        if tool_text:
            useful = [t for t in tool_text[-2:] if not t.startswith(refusal)]
