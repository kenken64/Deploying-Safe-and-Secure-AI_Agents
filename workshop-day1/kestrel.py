#!/usr/bin/env python3
"""Kestrel Goat - one command, three operating systems.
...
def cmd_setup(_argv: list[str]) -> int:
    ...
    print("    python kestrel.py attack a1    to watch the opening breach in the terminal")
    return 0
...
def cmd_reset(_argv: list[str]) -> int:
    ...
    print("  8 orders, 4 help-centre articles (one of them poisoned)")
    return 0
...
def cmd_controls(_argv: list[str]) -> int:
    ...
    print(f"{'control':<28} {'state':<6} {'block':<6} what it does")
    print("-" * 96)
    for key, meta in CONTROLS.items():
        print(f"{key:<28} {'ON' if settings.on(key) else 'off':<6} "
              f"{meta['block']:<6} {meta['label']}")
    print("-" * 96)
    print("Turn one on for a single run:   python kestrel.py attack a1 --control SECURE_TENANCY")
